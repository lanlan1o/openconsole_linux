...
{
    // The original header relies on the (non-standard) name injection of
    // "friend class SixelParser" to forward-declare the type. Clang does not
    // make that name visible for std::shared_ptr<SixelParser>, so declare it
    // explicitly. This is a duplicate-forward, which is fine.
    class SixelParser;
